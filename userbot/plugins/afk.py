...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"My Master Is Going Offline for sometime, and Reason is {reason}")
        else:
            await event.edit(f"My Master Is Going offline for sometime")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = f"My Master Has Been Gone For {afk_since}\nWhere He Is: Idk " + \
            f"\n\n__ He'll back whenever he want__\n**REASON**: {reason}" \
            if reason \
